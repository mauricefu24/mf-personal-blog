...
export default function CookingLibrary() {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[28px] border border-zinc-200 bg-[linear-gradient(180deg,_rgba(255,255,255,0.96)_0%,_rgba(255,237,213,0.88)_100%)] p-6 shadow-sm sm:p-8">
        <div className="max-w-3xl">
          <p className="text-sm font-medium uppercase tracking-[0.28em] text-orange-700">Recipe Search</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-3 sm:flex-row">
          <input
            ...
            className="h-13 flex-1 rounded-2xl border border-orange-200 bg-white px-5 text-base text-zinc-900 outline-none transition focus:border-orange-500 focus:ring-4 focus:ring-orange-100"
          />
          <button
            type="submit"
            disabled={loading}
            className="inline-flex h-13 items-center justify-center rounded-2xl bg-orange-500 px-6 text-sm font-semibold text-white transition hover:bg-orange-600 disabled:cursor-not-allowed disabled:bg-orange-300"
          >
            {loading ? "搜索中..." : "搜索菜谱"}
          </button>
        </form>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          {quickSearches.map((keyword) => (
            <button
              ...
              className="rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm font-medium text-zinc-700 transition hover:border-orange-200 hover:bg-orange-50 hover:text-orange-700"
            >
              {keyword}
            </button>
          ))}
      ...
      <section className="rounded-[28px] border border-zinc-200 bg-white/90 p-5 shadow-[0_24px_70px_rgba(15,23,42,0.06)] sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-4 border-b border-zinc-200 pb-5">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.24em] text-zinc-500">Recipe Result</p>
        ...
        {meals.length > 0 ? (
          <div className="mt-6 grid gap-5">
            {meals.map((meal) => (
              <article
                key={meal.id}
                className="grid gap-5 rounded-[26px] border border-zinc-200 bg-[linear-gradient(180deg,_#ffffff_0%,_#fff7ed_100%)] p-5 shadow-sm lg:grid-cols-[220px_1fr]"
              >
                <div className="overflow-hidden rounded-[20px] border border-zinc-200 bg-zinc-100">
                  {meal.imageUrl ? (
                    ...
                    <div className="flex h-full min-h-[220px] items-center justify-center bg-[linear-gradient(180deg,_#ffedd5_0%,_#fed7aa_100%)] px-6 text-center text-sm font-medium leading-7 text-orange-800">
                      暂无图片
                    </div>
                  )}
                </div>
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <p className="text-sm uppercase tracking-[0.24em] text-orange-700">Recipe</p>
                      <h4 className="mt-3 text-3xl font-semibold tracking-tight text-zinc-950">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {meal.youtubeUrl ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex h-11 items-center justify-center rounded-2xl bg-orange-500 px-5 text-sm font-medium text-white transition hover:bg-orange-600"
                        >
                          看视频
                        </a>
                      ) : null}
                  ...
                  <div className="mt-6 grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
                    <div className="rounded-[22px] border border-orange-100 bg-orange-50 p-5">
                      <p className="text-sm font-semibold uppercase tracking-[0.2em] text-orange-700">食材清单</p>
                    ...
                    <div className="rounded-[22px] border border-zinc-200 bg-white p-5">
                      <p className="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-500">做法步骤</p>
                        ...
                        {shortenInstructions(meal.instructionsZh).map((step, index) => (
                          <div key={`${meal.id}-step-${index}`} className="space-y-1">
                            ...
                            {meal.instructions[index] ? (
                              <p className="text-xs leading-6 text-zinc-400">{meal.instructions[index]}</p>
                            ) : null}
                          </div>
                        ))}
                      </div>
